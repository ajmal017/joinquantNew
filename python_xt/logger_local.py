...
class logger_xt:
    ...
    def __init__(self,set_level='info',file_path=log_path,use_console=True):
        """
        ...
        if  not self.logger.handlers:
            ...
            log_handler.setFormatter(logging.Formatter('%(asctime)s - %(pathname)s[:%(lineno)d] -  %(name)s - %(levelname)s: %(message)s'))
            self.logger.addHandler(log_handler)
            if use_console:
                console_handler = logging.StreamHandler()
                console_handler.setFormatter(logging.Formatter('%(asctime)s - %(pathname)s[:%(lineno)d] -  %(name)s - %(levelname)s: %(message)s'))
